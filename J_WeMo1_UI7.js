...
var WeMo = (function (api) {
	...
	function configurationAddFoundDevice(device, index, button, static)
	{
		...
		var unknownDeviceName = api.getDeviceState(device, "urn:futzle-com:serviceId:WeMo1", "UnknownDevice" + index + "Name", 0);
		var unknownDeviceType = api.getDeviceState(device, "urn:futzle-com:serviceId:WeMo1", "UnknownDevice" + index + "Type", 0);
		var unknownDeviceUSN = api.getDeviceState(device, "urn:futzle-com:serviceId:WeMo1", "UnknownDevice" + index + "USN", 0);
		var unknownDeviceHost = api.getDeviceState(device, "urn:futzle-com:serviceId:WeMo1", "UnknownDevice" + index + "Host", 0);
		if (static)
		{
	...
	{
		var btn = jQuery(button);
		var name = btn.parent().find('.wemo_name').val();
		var type = btn.parent().find('.wemo_type').val();
		var host = btn.parent().find('.wemo_host').val();
	...
	function configuration(device){
      ...
      html += '<p id="status_display" style="width:100%; position:relative; margin-left:auto; margin-right:auto; table-layout:fixed; text-align:center; color:black"></div>';
			...
			for (i = 1; i <= childDevices; i++)
			{
				...
				childHtml += '<td><input type="button" value="Remove" onClick="WeMo.configurationRemoveChildDevice(' + device + ',' + i + ',this)"/></td>';
				childHtml += '</tr>';
				actualChildDevices++;
			}
			childHtml += '</table>';
			...
			html += '<p><input type="checkbox"' +
				...
				' onclick="WeMo.setEnableMulticast(' + device + ', this)">&#xA0;Enable scan for WeMo devices on LAN</p>';
			...
			if (enableMulticast == "1")
			{
				...
				for (i = 1; i <= unknownDevices; i++)
				{
					...
					html += '<td><input type="button" value="Add Dynamic" onClick="WeMo.configurationAddFoundDevice('
					 + device + ',' + i + ',this,false)"/>&#xA0;';
					html += '<input type="button" value="Add Static" onClick="WeMo.configurationAddFoundDevice('
			...
			{
				html += '<div style=" margin: 5px;"><p>UPnP Proxy running (API version ' + wemoEscapeHtml(proxyApiVersion) + ')</p></div>';
			}
